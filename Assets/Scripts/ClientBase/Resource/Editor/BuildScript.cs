...
namespace CocoonAsset.Editor
{
    public static class BuildScript
    {
        ...
        private static string _CreateAssetBundleDirectory()
        {
            // Choose the output path according to the build target.

            //string outputPath = Path.Combine(AssetBundleUtility.AssetBundlesOutputPath, AssetBundleUtility.GetPlatformName());
            if (!Directory.Exists(AssetBundleUtility.AssetBundlesOutputPath))
                Directory.CreateDirectory(AssetBundleUtility.AssetBundlesOutputPath);

            return AssetBundleUtility.AssetBundlesOutputPath;
        }

        public static void BuildAssetBundles(List<AssetBundleBuild> builds)
        {
            // Choose the output path according to the build target.
            string outputPath = _CreateAssetBundleDirectory();
        ...
        public static void BuildStandalonePlayer()
        {
            ...
            CopyAssetBundlesTo(Path.Combine(Application.streamingAssetsPath, AssetBundleUtility.AssetBundlesOutputPath));
        ...
        static public void CopyAssetBundlesTo(string outputPath)
        {
            ...
            string sourcePath = Path.Combine(System.Environment.CurrentDirectory, AssetBundleUtility.AssetBundlesOutputPath);
            //string sourcePath = Application.dataPath + "/../AssetBundles";
            if (!Directory.Exists(sourcePath))
                Debug.Log("No assetBundle output folder, try to build the assetBundles first.");

            foreach (string dir in Directory.GetDirectories(sourcePath, "*", SearchOption.AllDirectories))
            {
                Directory.CreateDirectory(dir.Replace(sourcePath, outputPath));
            }
            foreach (string file in Directory.GetFiles(sourcePath, "*.*", SearchOption.AllDirectories))
            {
                File.Copy(file, file.Replace(sourcePath, outputPath), true);
            }
        ...
        {
            return AssetBundleUtility.AssetBundlesOutputPath + ".manifest";
        }
